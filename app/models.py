...
class Material(BaseModel):
    ...
    def sanitize_string_inputs(cls, v, info):
        ...
        if isinstance(v, str):
            if "\x00" in v:
                message = f"{info.field_name} contains invalid null bytes"
                raise ValueError(message)

            stripped = v.strip()
            if not stripped:
                message = f"{info.field_name} cannot be only whitespace"
                raise ValueError(message)
